...
uses
   ...
   cpc_statements_unit, cpc_core_objects_unit, filectrl, System.RegularExpressions;
...
procedure TMainForm.FileOpenMenuItemClick(Sender: TObject);
   var
      ...
      radio_button: TRadioButtonWithValue;
   ...
   begin
      if OpenDialog.Execute then
         begin
            pic := '';
            regex.Create (' ([^ _]+)_configuration_bits: ');
            ...
            while not eof (f) do
               begin
                  ...
                  m := regex.Match(s);
                  if m.Success then
                     pic := UpperCase(m.Groups[1].Value)
               end;
            CloseFile(f);
            if pic = '' then
               begin
   ...
   begin
      ...
      if SelectFolder (dir, 'Select Folder to save ' + config_bits_constant_name(pic) + '.inc') then
         begin
            ...
            then
               dir := dir + '\';
            include_file_name := dir + config_bits_constant_name(pic) + '.inc';
            FileSaveMenuItem.Enabled := true;
            Memo.Lines.SaveToFile (include_file_name);
            dirty := false;
            set_caption
