UNIT pic18x_xml_parser_unit;

INTERFACE

uses pic18x_xml_information_unit;

function get_pic18x_xml_into (fn: string): TPICXMLInfo;

IMPLEMENTATION

uses
   System.Classes, SAXComps, SAX, BSAX, SAXKW, SAXAdapters, System.SysUtils, wirth_balanced_binary_tree_unit;
...
type
   ...
   TPICXMLParser =
      ...
      public
         ...
         SAXErrorHandler: TSAXErrorHandler;
         SAXContentHandler, SAXIncludedContentHandler: TSAXContentHandler;
         constructor Create;
            reintroduce;
         procedure SAXErrorHandler1Error (Sender: TObject; const Error: ISAXParseError);
         procedure SAXErrorHandler1FatalError (Sender: TObject; const Error: ISAXParseError);
         procedure SAXErrorHandler1Warning (Sender: TObject; const Error: ISAXParseError);

         procedure SAXContentHandlerStartDocument (Sender: TObject);
         procedure SAXContentHandlerStartElement (Sender: TObject; const NamespaceURI, LocalName, QName: String; const Atts: IAttributes);
         procedure SAXContentHandlerEndElement (Sender: TObject; const NamespaceURI, LocalName, QName: String);
         procedure SAXContentHandlerEndDocument (Sender: TObject);

         procedure SAXContentHandlerStartIncludedDocument (Sender: TObject);
         procedure SAXContentHandlerEndIncludedDocument (Sender: TObject);
...
constructor TPICXMLParser.Create;
   begin
      SAXErrorHandler := TSAXErrorHandler.Create (self);
      SAXErrorHandler.OnWarning := SAXErrorHandler1Warning;
      SAXErrorHandler.OnError := SAXErrorHandler1Error;
      SAXErrorHandler.OnFatalError := SAXErrorHandler1FatalError;

      SAXContentHandler := TSAXContentHandler.Create (self);
      SAXContentHandler.OnStartDocument := SAXContentHandlerStartDocument;
      SAXContentHandler.OnStartElement := SAXContentHandlerStartElement;
      SAXContentHandler.OnEndElement := SAXContentHandlerEndElement;
      SAXContentHandler.OnEndDocument := SAXContentHandlerEndDocument;

      SAXIncludedContentHandler := TSAXContentHandler.Create (self);
      SAXIncludedContentHandler.OnStartDocument := SAXContentHandlerStartIncludedDocument;
      SAXIncludedContentHandler.OnStartElement := SAXContentHandlerStartElement;
      SAXIncludedContentHandler.OnEndElement := SAXContentHandlerEndElement;
      SAXIncludedContentHandler.OnEndDocument := SAXContentHandlerEndIncludedDocument
   end;

procedure TPICXMLParser.SAXErrorHandler1Error (Sender: TObject;
  const Error: ISAXParseError);
   begin
      assert (false, '[Error] ' + Error.getMessage + ' Line ' +
        IntToStr(Error.getLineNumber) + ' Column ' +
        IntToStr(Error.getColumnNumber));
   end;

procedure TPICXMLParser.SAXErrorHandler1FatalError (Sender: TObject;
  const Error: ISAXParseError);
   begin
      assert (false, '[Fatal Error] ' + Error.getMessage + ' Line ' +
        IntToStr(Error.getLineNumber) + ' Column ' +
        IntToStr(Error.getColumnNumber));
   end;

procedure TPICXMLParser.SAXErrorHandler1Warning (Sender: TObject;
  const Error: ISAXParseError);
   begin
      assert (false, '[Warning] ' + Error.getMessage + ' Line ' +
        IntToStr(Error.getLineNumber) + ' Column ' +
        IntToStr(Error.getColumnNumber));
   end;

procedure TPICXMLParser.parse (FileName: string);
   var
...
procedure TPICXMLParser.SAXContentHandlerStartDocument (Sender: TObject);
   begin
...
procedure TPICXMLParser.SAXContentHandlerStartElement (Sender: TObject; const NamespaceURI, LocalName, QName: String; const Atts: IAttributes);

   function get_attrs (attr_name: string): string;
      begin
         assert (pos('.', attr_name) = 0, 'bad attr_name (with "."): ' + attr_name);
         assert (Atts <> nil);
         result := Atts.getValue (attr_name)
      end;
   ...
   function get_attri (attr_name: string): integer;
      var
         ...
         i: integer;
      begin
         assert (pos('.', attr_name) = 0, 'bad attr_name (with "."): ' +
           attr_name);
         assert (Atts <> nil);
         result := 0;
         v := Atts.getValue (attr_name);
      ...
      begin
         ...
         repeat
            ...
            if CharInSet (s[i], ['0'..'9']) then
               ...
               result := result + 10 + ord(s[i]) - ord('a');
            i := i + 1
         until not CharInSet (s[i], hex_chars)
      end;
   var
      i,j: integer;
      field: TSFRField;
   ...
   begin
      level := level + 1;
      stk[level].tag := identify_tag (QName);
      stk[level].QName := QName;
      stk[level].ptr := nil;
            ...
            other_tag:
               begin
                  if (stk[level-1].tag in [RegardlessOfMode, ExtendedModeOnly])
                     ...
                     (QName <> 'edc:NMMRPlace')
                     and
                     (QName <> 'edc:EmulatorZone')
                  then
                     assert (false, 'unexpected element in DataSpace???? ' + QName);
               end;
            AdjustPoint:
...
//                     field.width := get_attri ('edc:nzwidth');
                     case stk[level-3].tag of
                        JoinedSFRDef:
                           begin
//                              jsfr := TJoinedSFR (stk[level-3].ptr);
//                              i := Length(jsfr.joined_fields);
//                              SetLength (jsfr.joined_fields, i+1);
//                              jsfr.joined_fields[i] := field
                           end;
                        SFRDef:
                           begin
                     field := TSFRField.Create;
...
procedure TPICXMLParser.SAXContentHandlerEndElement (Sender: TObject; const NamespaceURI, LocalName, QName: String);
   var
      joined_sfr_info: TJoinedSFR;
      sfr0, sfr1: TSFRDef;
      t: TSFR;
   begin
      assert (stk[level].tag = identify_tag(QName));
      case stk[level].tag of
         JoinedSFRDef:
            begin
...
procedure TPICXMLParser.SAXContentHandlerEndDocument (Sender: TObject);
   begin
      assert (level = -1)
   end;

procedure TPICXMLParser.SAXContentHandlerStartIncludedDocument (Sender: TObject);
   begin
   end;

procedure TPICXMLParser.SAXContentHandlerEndIncludedDocument (Sender: TObject);
   begin
   end;
...
function get_pic18x_xml_into (fn: string): TPICXMLInfo;
   begin
      result := TPICXMLParser.Create;
      TPICXMLParser(result).parse (fn)
   end;
