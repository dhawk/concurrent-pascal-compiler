...
procedure test111;
   begin   //   technically UFRM is read-only, but this simulator doesn't know that...
...
procedure test137;
   ...
   begin
      ...
      add ('   result := b');
      add (' end;');
      add ('function fbb (a,b: boolean): boolean;');
      add (' begin');
      add ('   if a and b then');
      add ('     result := true');
      add (' end;');
      add ('function ffb (a,b: boolean): boolean;');
      add (' begin');
      add ('   if f(a) and b then');
      add ('     result := true');
      add (' end;');
      add ('function fbf (a,b: boolean): boolean;');
      add (' begin');
      add ('   if a and f(b) then');
      add ('     result := true');
      add (' end;');
      add ('function fbl (a,b: boolean): boolean;');
      add ('  var i,j: uint1;');
      add ('  begin');
      add ('    if a then i := 1;');
      add ('    if b then j := 1;');
      add ('    if (i=1) and (j=1) then');
      add ('      result := true');
      add ('  end;');
      add ('function fbr (a,b: boolean): boolean;');
      add ('  var i,j: uint1;');
      add ('  begin');
      add ('    if a then i := 1;');
      add ('    if b then j := 1;');
      add ('    if (1=i) and (1=j) then');
      add ('      result := true');
      add ('  end;');
      add ('function fbln (a,b: boolean): boolean;');
      add ('  var i,j: uint1;');
      add ('  begin');
      add ('    if not a then i := 1;');
      add ('    if not b then j := 1;');
      add ('    if (i=0) and (j=0) then');
      add ('      result := true');
      add ('  end;');
      add ('function fbrn (a,b: boolean): boolean;');
      add ('  var i,j: uint1;');
      add ('  begin');
      add ('    if not a then i := 1;');
      add ('    if not b then j := 1;');
      add ('    if (0=i) and (0=j) then');
      add ('      result := true');
      add ('  end;');
      add ('var');
      ...
      add (' b16,b17,b18,b19: boolean;');
      add (' b20,b21,b22,b23: boolean;');
      add ('');
      add (' b24,b25,b26,b27: boolean;');
      add (' b28,b29,b30,b31: boolean;');
      add (' b32,b33,b34,b35: boolean;');
      add ('');
      add (' b36,b37,b38,b39: boolean;');
      add (' b40,b41,b42,b43: boolean;');
      add (' b44,b45,b46,b47: boolean;');
      add ('');
      add (' b48,b49,b50,b51: boolean;');
      add (' b52,b53,b54,b55: boolean;');
      add (' b56,b57,b58,b59: boolean;');
      add ('');
      add ('');
      add ('begin');
      add (' b0 := fbb (false, false);');
      add (' b1 := fbb (false, true);');
      add (' b2 := fbb (true, false);');
      add (' b3 := fbb (true, true);');
      add (' b4 := fbb (false, false);');
      add (' b5 := fbb (false, true);');
      add (' b6 := fbb (true, false);');
      add (' b7 := fbb (true, true);');
      add (' b8 := fbb (false, false);');
      add (' b9 := fbb (false, true);');
      add (' b10 := fbb (true, false);');
      add (' b11 := fbb (true, true);');
      add ('');
      add (' b12 := fbl (false, false);');
      add (' b13 := fbl (false, true);');
      add (' b14 := fbl (true, false);');
      add (' b15 := fbl (true, true);');
      add (' b16 := fbl (false, false);');
      add (' b17 := fbl (false, true);');
      add (' b18 := fbl (true, false);');
      add (' b19 := fbl (true, true);');
      add (' b20 := fbl (false, false);');
      add (' b21 := fbl (false, true);');
      add (' b22 := fbl (true, false);');
      add (' b23 := fbl (true, true);');
      add ('');
      add (' b24 := fbr (false, false);');
      add (' b25 := fbr (false, true);');
      add (' b26 := fbr (true, false);');
      add (' b27 := fbr (true, true);');
      add (' b28 := fbr (false, false);');
      add (' b29 := fbr (false, true);');
      add (' b30 := fbr (true, false);');
      add (' b31 := fbr (true, true);');
      add (' b32 := fbr (false, false);');
      add (' b33 := fbr (false, true);');
      add (' b34 := fbr (true, false);');
      add (' b35 := fbr (true, true);');
      add ('');
      add (' b36 := fbln (false, false);');
      add (' b37 := fbln (false, true);');
      add (' b38 := fbln (true, false);');
      add (' b39 := fbln (true, true);');
      add (' b40 := fbln (false, false);');
      add (' b41 := fbln (false, true);');
      add (' b42 := fbln (true, false);');
      add (' b43 := fbln (true, true);');
      add (' b44 := fbln (false, false);');
      add (' b45 := fbln (false, true);');
      add (' b46 := fbln (true, false);');
      add (' b47 := fbln (true, true);');
      add ('');
      add (' b48 := fbrn (false, false);');
      add (' b49 := fbrn (false, true);');
      add (' b50 := fbrn (true, false);');
      add (' b51 := fbrn (true, true);');
      add (' b52 := fbrn (false, false);');
      add (' b53 := fbrn (false, true);');
      add (' b54 := fbrn (true, false);');
      add (' b55 := fbrn (true, true);');
      add (' b56 := fbrn (false, false);');
      add (' b57 := fbrn (false, true);');
      add (' b58 := fbrn (true, false);');
      add (' b59 := fbrn (true, true);');
      add ('');
      add ('end.');
      start_test (137);
      test_abs_value (0, 0);
...
procedure test138;
   begin
      ...
      add (' begin');
      add ('   x := true;');
      add ('   result := x and ((a=0) and (b=0))');
      add (' end;');
      add ('function f01 (a,b: uint1): boolean;');
      add (' var x: boolean;');
      add (' begin');
      add ('   x := true;');
      add ('   result := x and ((a=0) and (b=1))');
      add (' end;');
      add ('function f10 (a,b: uint1): boolean;');
      add (' var x: boolean;');
      add (' begin');
      add ('   x := true;');
      add ('   result := x and ((a=1) and (b=0))');
      add (' end;');
      add ('function f11 (a,b: uint1): boolean;');
      add (' var x: boolean;');
      add (' begin');
      add ('   x := true;');
      add ('   result := x and ((a=1) and (b=1))');
      add (' end;');
      add ('var');
      add (' b0,b1,b2,b3: boolean;');
      add (' b4,b5,b6,b7: boolean;');
...
procedure test141;
   begin   // test assign & push 1-bit ioreg param
           //   technically UFRM is read-only, this simulator doesn't know that...
           //   so this test won't work on a real chip
      add ('{$processor ''pic18f65j94''}');
      ...
      add ('     i := 1');
      add ('  end;');
      add ('procedure pclrc (ioreg i: uint1);');
      add ('  begin');
      add ('     i := 0');
      add ('  end;');
      add ('procedure psetv (ioreg i: uint1);');
      add ('  var j: uint1 := 1;');
      add ('  begin');
      add ('     i := j');
      add ('  end;');
      add ('procedure pclrv (ioreg i: uint1);');
      add ('  var j: uint1;');
      add ('  begin');
      add ('     i := j');
      add ('  end;');
...
procedure test142;
   begin  // test assign & push 1-bit ioreg param in alt addr sfr
      ...
      add ('     result := i');
      add ('  end;');
      add ('var i0,i1,i2,i3: uint8;');
      add ('begin');
      add ('  psetc (osccon.osts);');
      add ('  psetc (refocon.roon);');
      add ('  psetc (refocon.rosslp);');
      add ('  pclrc (refocon.rosslp);');
      add ('  psetv (refocon.rosel);');
      add ('  psetc (refocon.rodiv3);');
      add ('  pclrv (refocon.rodiv3);');
      add ('  i0 := f(osccon.osts);');
      add ('  i1 := f(refocon.roon);');
