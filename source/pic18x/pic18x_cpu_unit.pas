...
procedure TPIC18x_CPU.generate_machine_code (_prog: TProgram);
   ...
   procedure generate_ram_variable_map;
      ...
      procedure out_var_info (v: TPIC18x_Variable; this_ptr: integer; indent, dot: string);
               ...
               if TSystemType(v.typedef).permanent_ram_vars.Length > 0 then
                  begin
                     ...
                     for i := 0 to st.permanent_ram_vars.Length-1 do
                        out_var_info (TPIC18x_Variable(st.permanent_ram_vars[i]), 0, indent + indent_amt, '.')
                  end
            end;
         ...
         procedure out_systemtype_var_info (this_ptr: integer);
            ...
            begin
               ...
               if st.permanent_eeprom_vars.Length > 0 then
                  addr := addr - 1;
               TAssemblyComment.Create (format ('%3.3X %s%s%s  size=%d  this_ptr=$%3.3X', [addr, indent, dot, v.name, TPIC18x_TypeInfo(v.typedef.info).Size, this_ptr]));
               ...
               for i := st.parameters.Length-1 downto 0
                  do case st.parameters[i].address_mode of
                        system_type_address_mode:
                           out_var_info (TPIC18x_Variable(st.parameters[i]), this_ptr, indent, dot);
               ...
               for i := 0 to st.permanent_ram_vars.Length-1 do
                  out_var_info (TPIC18x_Variable(st.permanent_ram_vars[i]), this_ptr, indent, dot);
      ...
      begin
         ...
         for i := 0 to prog.program_vars.Length-1 do
            out_var_info (TPIC18x_Variable(prog.program_vars[i]), 0, '', '');
